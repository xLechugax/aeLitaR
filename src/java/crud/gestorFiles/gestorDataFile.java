...
public class gestorDataFile {
    ...
    public ArrayList<ImagenVO> Listar_DataFile_idProcedimiento(int idProcedimiento, int idEmpresa) {
        ArrayList<ImagenVO> list = new ArrayList<ImagenVO>();
        Connection conn = ConexionBD.getConexion();
        String sql = "SELECT * FROM archivo where archivo.idProcedimiento="+idProcedimiento+" and archivo.idEmpresa="+idEmpresa;
        ResultSet rs = null;
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement(sql);
    ...
    public void agregar_Imagen(ImagenVO dataVO) {
        ...
        if (dataVO.getidTrabajo() != 0) {
            String sql = "INSERT INTO `archivo` (`idEmpresa`, archivo, `nombre`, `type`, `idOrdenTrabajo`,`idTarea`,`idProcedimiento`) VALUES (?,?,?,?,?,?,?)";
            PreparedStatement ps = null;
            try {
                ps = conn.prepareStatement(sql);
                ...
                ps.setInt(7, dataVO.getidProcedimiento());
                ps.executeUpdate();
            } catch (SQLException ex) {
                System.out.println(ex.getMessage());
            } catch (Exception ex) {
