...
<%
    ...
    ResultSet rsUsuario, rsContadorEmpresas, rsListadoEmpresas, rsUsuarioCorreo = null;
    ...
    try {
        ...
        String sqlCorreo = "SELECT * from usuario WHERE email='" + nombreUsuario.toLowerCase() + "' and clave='" + clave + "'";
        PreparedStatement pstCorreo = conn.prepareStatement(sqlCorreo);
        rsUsuarioCorreo = pstCorreo.executeQuery();  // Crea una nueva sesión para el usuario actual.
...
<%          }
} else if (rsUsuarioCorreo.next() == true) {
    if (nombreUsuario.toLowerCase().equals(rsUsuarioCorreo.getString("email")) & clave.equals(rsUsuarioCorreo.getString("clave"))) { //Verifica si existe realmente
    
        String correo = rsUsuarioCorreo.getString("email").toLowerCase();
        String idUsuario = rsUsuarioCorreo.getString("idUsuario"); 
